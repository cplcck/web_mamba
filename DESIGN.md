...
- **Operator explanations:** every captured operator kind has a short Korean description beside its unchanged primary name in stage choices and search results. Selecting an operator shows a visible `입력 → 처리 → 출력` explanation before the existing tensor sections: actual operand names, GGML axis lengths, element counts and dtypes on both sides, a plain-language operation description, and relevant captured parameters/alias facts. Include all input and weight operands, not only previews. Shape equality must not imply numeric equality; reshape's padded four-axis shape may stay equal. Decode UNARY/GLU subtypes and scalar parameters from pinned GGML evidence. Never fabricate tensor values or row-index contents absent from the capture. Keep full metadata in the existing advanced disclosure and all overflow inside the inspector. Descriptions wrap at the existing 14px metadata size; no new fonts or colors.
- **Exclusive architecture selection:** Embedding, Mamba block, Final Norm and Output replace one another's detail view. The representative enters a presentation-only block-selection view, rooted at the actual model ID with `view=blocks` in the URL, not a fabricated captured entity. Only the 24 block choices and a block-selection instruction remain: previous block/stage operators, graph evidence and inspector tensors are removed. No block is implicitly selected. Choosing a real block returns to its normal details. Collapsing the chooser does not restore the previous selection. Scenario/history/reload preserve the appropriate view, and compact block selection never auto-opens a modal over the choices.
- **Block selector dismissal and operator totals:** activating any other button or native disclosure closes the expanded Mamba block choices, including reselection and controls outside the explorer. The representative button remains a toggle; closing choices preserves the selected entity and its stage detail. Selection navigation starts the block choices closed. Block overview buttons and stage headings explicitly show `total operators : N`, counting all captured descendant operator entities rather than stages or distinct operation names.
- **Operator display names:** every operator's primary visible name is its lowercase operation name only, such as `rms_norm` or `mul`. The exported ID format is `<stage>/<operation>.<capture index>`; omit the stage prefix and terminal numeric index from names while retaining the stage heading as context. Apply this to choices, breadcrumbs, search names, selected titles/status, graph nodes, and visible edge endpoints. Repeated operation names remain distinct actions with original IDs, routes, descriptions, tensor data and metadata. Original IDs/opcodes remain searchable; raw identity match captions are not repeated as display names. Explicit advanced evidence retains canonical IDs and execution parameters.
...
- **Horizontal panel resizing:** a vertical, focusable separator between the panes moves horizontally only. Dragging left widens the right pane; dragging right narrows it. The default remains 360px, with `--inspector-width-min:320px` and `--explorer-width-min:400px`; the maximum is derived from available workspace width. A dedicated `--pane-splitter-width:44px` track provides a full-height pointer target around a thin visible grip. Pointer capture, cancellation, capture loss, window blur and compact transitions terminate the gesture without a stuck cursor or text selection. Width is retained across selections and in-page responsive transitions, not persisted across reloads.
- **Separator accessibility and reflow:** `role=separator`, vertical orientation, an accessible name referencing the selection-info heading, `aria-controls`, and live min/max/current width plus pixel-valuetext. Left/Right move the boundary by `--space-4`; Home/End set the right pane to minimum/maximum width. Up/Down do not resize it. The separator is hidden in compact drawer mode. The explorer's own width, not only the viewport, controls compact model-card typography through a container query. Existing independent panel scrolling is unchanged. Mechanism references: [WAI window splitter](https://raw.githubusercontent.com/w3c/aria-practices/main/content/patterns/windowsplitter/windowsplitter-pattern.html) and [beui range slider](https://beui.dev/r/range-slider/raw); use immediate native pointer updates without spring animation or added dependencies.
- **Compact selection summary:** selected kind, display name, scenario and a short Korean role explanation, followed by separate Inputs, Outputs, Weights and relevant State groups. Each boundary keeps its actual total count; preview at most two main activation tensors (index inputs only when there is no activation), with state/index counts explicitly distinguished. Preview values are original name, native `ne[4]`, dtype and compact logical bytes. Weights show actual count and unique GGUF payload bytes, never intermediate/peak-memory claims. Model summaries never borrow a block selection; global weights and state counts describe the model, not block 0.
...
The primary flow is vertical at every viewport: input tensor cards above operator/entity cards, then output tensor cards below. Weight-role incoming tensors sit in a right-side column aligned with their first actual consuming entity, never as a step above the operator. Shared weights keep one boundary card and every actual consumer edge. Each operator row reserves enough height for its wrapped labels and attached weight stack; compact views retain the same side-input relationship with wrapping rather than smaller text. One-input operator paths are straight down the shared center axis. Nonadjacent and recurrent connections use a gutter to avoid unrelated cards. Solid lines retain role colors and text labels; each actual edge has one uniform open arrow indicating direction, down into primary cards and left into side-fed operators. No role-specific symbols or shape legend are rendered. Natural width is capped at 960px and available width; fit uses available width; reset preserves selection, data, and disclosure state. Named graphGeometry values are inset=16px, cardInset=12px, sideGap=24px, rowGap=32px, portGap=16px, arrowOffset=8px, minCardHeight=72px, maxCardWidth=320px, naturalWidth=960px. Required text remains 14px and names wrap without truncation.

Role names remain on tensor cards, accessible edge labels and the complete edge list, so color is never the only way to identify data. Arrows indicate direction only, not tensor type.
