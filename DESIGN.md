...
- **Operator display names:** every operator's primary visible name is its actual parent stage path only, such as `model/final-normalization`. Apply this to choices, breadcrumbs, search names, selected titles/status, graph nodes, and visible edge endpoints. Same-stage operators intentionally share a name but remain distinct actions with original IDs, routes, descriptions, tensor data and metadata. Original IDs/opcodes remain searchable; raw identity match captions are not repeated as display names. Explicit advanced evidence retains canonical IDs.
