...
## Current structural contract — representative Mamba block
...
- **One common outline:** a full-width `Mamba block × 24` header represents shared structure, not shared weights or state. Its native button starts with `aria-expanded=false`; activating it reveals 24 actual block choice buttons in a bounded region. Choosing a block selects that exact capture entity and closes the choices, returning focus to the header. No 24 cloned stage graphs.
- **Real data:** no-hash landing selects actual `block.0`. The active data block is named explicitly. Exact model/block/stage/operator hashes, scenario preservation, nearest-ancestor fallback, and browser history retain their original meanings. Model-wide stages remain reachable via breadcrumbs and a compact model-context disclosure.
