...
async function checkFlow(page: Page, scenario: 'prefill' | 'decode', entityId: string): Promise<void> {
  ...
  const actual = await page.evaluate(() => {
    ...
    const markerFailures = [...svg.querySelectorAll('.edge-marker')].flatMap(marker => {
      const box = marker.getBoundingClientRect();
      const edgeKey = marker.parentElement?.querySelector('.graph-edge')?.getAttribute('data-edge-key');
    ...
    return {
      ...
      legendRoles: [...document.querySelectorAll<HTMLElement>('.graph-legend__item')].map(item => item.dataset.edgeRole),
      markerCount: svg.querySelectorAll('.edge-marker').length,
      markerFailures,
      cardOverlaps,
      clippedText,
  ...
  expect(actual.legendRoles.sort()).toEqual([...new Set(model.edges.map(edge => edge.role))].sort());
  expect(actual.markerCount).toBe(model.edges.length);
  expect(actual.markerFailures).toEqual([]);
...
for (const viewport of [{ width: 1440, height: 900 }, { width: 768, height: 1024 }, { width: 390, height: 844 }]) {
  test(`RMSNorm shows its real boundary tensors and unobscured markers at ${viewport.width}`, async ({ page }, info) => {
    ...
    // Then: input/operator/output are distinct, with no invented weight/state legend entries.
...
test('model, block and stage flows preserve every boundary and actual role in both scenarios', async ({ page }) => {
  // Given: larger graphs with shared inputs, weights, state and internal connections.
  ...
  for (const scenario of ['prefill', 'decode'] as const) {
    for (const entityId of ['mamba-130m', 'block.23', 'block.23/selective-scan-state', 'model/final-normalization']) {
      // When / Then: rendering preserves every identity and keeps each marker outside cards.
